...
class InventoryRefresher:
    ...
    def extract_car_info(self, title):
        ...
        brands = ['FISKER', 'BMW', 'MERCEDES-BENZ', 'MERCEDES', 'AUDI', 'FORD', 'CHEVROLET', 'TOYOTA', 'HONDA', 
                  'NISSAN', 'HYUNDAI', 'KIA', 'VOLKSWAGEN', 'LEXUS', 'ACURA', 'INFINITI',
                  'CADILLAC', 'LINCOLN', 'BUICK', 'GMC', 'JEEP', 'CHRYSLER', 'DODGE', 'RAM',
                  'SUBARU', 'MAZDA', 'MITSUBISHI', 'VOLVO', 'JAGUAR', 'LAND ROVER', 'PORSCHE',
        ...
        try:
            ...
            if brand != "Unknown Make":
                ...
                after_brand = re.sub(r'^\s*(OEM|ORIGINAL|GENUINE)\s+', '', after_brand, flags=re.IGNORECASE)
                ...
                for word in words[:4]:
                    ...
                    if (not re.match(r'\b(19|20)\d{2}\b', word) and 
                        word.upper() not in ['OEM', 'ORIGINAL', 'GENUINE', 'USED', 'NEW'] and
                        len(word) > 1 and
                        not word.isdigit()):
    ...
    def extract_item_details(self, article, headers):
        ...
        try:
            ...
            link_element = article.find('a', class_='str-item-card__property-title-link')
            if not link_element:
                return None, []
    ...
    def is_valid_part_title(self, title):
        ...
        part_keywords = [
            'OEM', 'PART', 'ASSEMBLY', 'MODULE', 'SENSOR', 'PUMP', 'FILTER', 'BRAKE',
            'ENGINE', 'TRANSMISSION', 'WHEEL', 'RIM', 'HEADLIGHT', 'TAILLIGHT',
            'MIRROR', 'DOOR', 'BUMPER', 'FENDER', 'HOOD', 'TRUNK', 'SEAT',
            'DASHBOARD', 'STEERING', 'SUSPENSION', 'EXHAUST', 'RADIATOR',
            'ALTERNATOR', 'STARTER', 'BATTERY', 'WIRE', 'HARNESS', 'ECU', 'ECM'
        ]
    ...
    def scrape_single_page(self, page_url, headers, current_page):
        ...
        try:
            ...
            logging.info(f"📡 Page {current_page} response status: {response.status_code}")
            
            if response.status_code != 200:
                logging.error(f"Failed to retrieve page {current_page}: {response.status_code}")
                return False, []
            ...
            for i, article in enumerate(articles):
                try:
                    ...
                    logging.error(f"Error processing article {i} on page {current_page}: {e}")
                    continue

            logging.info(f"✅ Scraped {len(page_parts)} valid items from page {current_page}")
            ...
            if len(page_parts) > 0:
                ...
                pagination_links = soup.find_all('a', {'aria-label': re.compile(r'Page \d+')})
                max_page_found = current_page
                for link in pagination_links:
                    try:
                        page_match = re.search(r'Page (\d+)', link.get('aria-label', ''))
                        if page_match:
                ...
                has_next_page = any(indicator for indicator in next_page_indicators) or current_page < max_page_found
    ...
    def scrape_ebay_parts(self, max_pages: int = 20):
        ...
        try:
            ...
            if response.status_code != 200:
                logging.error(f"❌ Cannot access eBay store. Status: {response.status_code}")
                return False
            ...
            if not articles:
                logging.error("❌ No items found on eBay page. Structure may have changed.")
                return False
            ...
            while page <= max_pages:
                ...
                delay = random.uniform(3, 6)  # Slightly longer delay for detailed scraping
            ...
            logging.info(f"✅ eBay scraping completed! Total parts: {len(self.all_parts_data)}")
    ...
    def save_combined_listing_csv(self):
        ...
        try:
            ...
            headers = [
                '*Title', 'Scraped Price', 'Original Price', 'PicURL', '*Description',
                'Part Number', 'Car Brand', 'Manufacturer Part Number', 'Year', 'Car Model',
                'Stock Number', 'VIN Number', 'NumbeVINr', '*Category', '*ConditionID',
                '*Format', '*Duration', '*StartPrice', '*Location', 'ShippingType',
                'ShippingService-1:Option', 'ShippingService-1:Cost', '*DispatchTimeMax',
                '*ReturnsAcceptedOption', 'Compatibilities'
            ]
            ...
            with open(output_path, 'w', newline='', encoding='utf-8') as csvfile:
                ...
                for part in self.all_parts_data:
                    row = [
                        ...
                        part.get('shipping_service_1_option', ''), # ShippingService-1:Option
                        part.get('shipping_service_1_cost', ''),  # ShippingService-1:Cost
                        part.get('dispatch_time_max', ''),        # *DispatchTimeMax
                        part.get('returns_accepted_option', ''),  # *ReturnsAcceptedOption
                        part.get('compatibilities', '')           # Compatibilities
                    ]
                    writer.writerow(row)

            logging.info(f"✅ Combined listing CSV saved with {len(self.all_parts_data)} parts")
            return True
        ...
        for part in self.all_parts_data:
            try:
                ...
                part_entry = {
                    ...
                    "shipping_option": part.get('shipping_service_1_option', 'USPSFirstClass'),
                    "shipping_cost": float(part.get('shipping_service_1_cost', '29.98'))
                }

                inventory[brand][model][year].append(part_entry)

            except Exception as e:
                logging.error(f"Error organizing part '{part.get('title', 'Unknown')}': {e}")
                continue

        self.inventory_data = inventory
        logging.info(f"✅ Organized {len(inventory)} brands with {len(self.all_parts_data)} total parts")
        return True
    ...
    def process_vin_matching(self):
        ...
        try:
            ...
            if os.path.exists(vin_match_path):
                result = subprocess.run(['python', vin_match_path], 
                                      cwd=self.base_path,
            ...
            if os.path.exists(vinwebscrape_path):
                result = subprocess.run(['python', vinwebscrape_path], 
                                      cwd=self.base_path,
                                      capture_output=True, text=True)
                if result.returncode == 0:
                ...
                logging.warning("⚠️ VIN web scrape script not found, skipping VIN scraping")
    ...
    def run_full_refresh(self):
        ...
        logging.info(f"   • Files updated: combined_listing.csv, grouped_by_vin.csv, donor_car.csv, inventory.json")
