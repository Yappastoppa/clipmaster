...
def limit_images(pic_url, max_images=6):
    """Limit the number of images in the PicURL field."""
    urls = pic_url.split('|')  # Updated delimiter to match the provided data
    return '|'.join(urls[:max_images])  # Use '|' to join limited URLs
    ...
    with open(original_csv, mode='r', encoding='utf-8') as file:
        ...
        for row in reader:
            ...
            if all(word in title.lower() for word in keywords):
                ...
                row['PicURL'] = limit_images(row['PicURL'])  # Limit the number of images
...
def create_csv_from_listings(listings, new_csv, fieldnames):
    ...
    with open(new_csv, mode='w', newline='', encoding='utf-8') as file:
        ...
        for part in tqdm(listings, desc="Writing items"):
            # Ensure the title, description, and PicURL are clean and properly processed
            part['*Title'] = truncate_title(part['*Title'])
            part['*Description'] = clean_text(part['*Description'])
            part['PicURL'] = limit_images(part['PicURL'])  # Ensure PicURL is limited
...
create_csv_from_listings(sorted_listings, new_csv, fieldnames=[
